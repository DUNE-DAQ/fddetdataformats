...
// NOSABOTAGELINTBEGIN(modernize-avoid-c-arrays)
  ...
  WordType get_adc_2d_as_1d(const int i_adc, const int i_channel, const WordType (&adc_matrix)[NWords]) { // NOLINT(modernize-avoid-c-arrays)
  ...
  template<typename WordType, int NWords, int BitsPerADC, int ADCSPerChannel, int NChannels>
  void set_adc_2d_as_1d(const int i_adc, const int i_channel, const WordType adc_val, WordType (&adc_matrix)[NWords]) { // NOLINT(modernize-avoid-c-arrays)
    static_assert(std::is_integral_v<WordType> &&
		  std::is_unsigned_v<WordType>,
		  "WordType must be an unsigned integral type");
	...
	throw std::out_of_range(std::format("Requested ADC value of {} exceeds max value of {}", adc_val, (static_cast<WordType>(1) << BitsPerADC) - 1));
      }
    }
  ...
  WordType get_adc_1d(const int i_adc, const WordType (&adc_array)[NWords]) { // NOLINT(modernize-avoid-c-arrays)
  ...
  WordType get_adc_2d(const int i_sample, const int i_adc, const WordType (&adc_matrix)[Rows][Columns]) { // NOLINT(modernize-avoid-c-arrays)
  ...
  template<typename WordType, int NWords, int BitsPerADC>
  void set_adc_1d(const int i_adc, WordType adc_val, WordType (&adc_array)[NWords]) { // NOLINT(modernize-avoid-c-arrays)
    static_assert(std::is_integral_v<WordType> &&
		  std::is_unsigned_v<WordType>,
		  "WordType must be an unsigned integral type");
  ...
  template<typename WordType, int Rows, int Columns, int BitsPerADC>
  void set_adc_2d(const int i_sample, const int i_adc, WordType adc_val, WordType (&adc_matrix)[Rows][Columns]) { // NOLINT(modernize-avoid-c-arrays)
    if (i_sample < 0 || i_sample >= Rows) {
      throw std::out_of_range(std::format("Requested index of {}th 1-d ADC array is outside of allowed range 0-{}", i_sample, Rows - 1));
    }
