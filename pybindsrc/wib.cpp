...
register_wib(py::module& m)
{
  ...
  py::class_<WIBFrame>(m, "WIBFrame", py::buffer_protocol())
    ...
    .def("get_wib_header",
         static_cast<const WIBHeader* (WIBFrame::*)() const>(&WIBFrame::get_wib_header),
    ...
    .def("get_channel",
         static_cast<uint16_t (WIBFrame::*)(const uint8_t, const uint8_t, const uint8_t) const>(&WIBFrame::get_channel))
    ...
    .def("get_bytes",
         [](WIBFrame* fr) -> py::bytes { return py::bytes(reinterpret_cast<char*>(fr), sizeof(WIBFrame)); });
  ...
  py::class_<ColdataHeader>(m, "ColdataHeader")
    ...
    .def_property_readonly("coldata_convert_count",
                           [](ColdataHeader& self) -> uint32_t { return self.coldata_convert_count; })
  ...
  py::class_<ColdataSegment>(m, "ColdataSegment").def("get_channel", &ColdataSegment::get_channel);
  py::class_<ColdataBlock>(m, "ColdataBlock").def("get_channel", &ColdataBlock::get_channel);
}
