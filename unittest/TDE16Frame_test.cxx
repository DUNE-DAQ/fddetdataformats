...
{
  TDE16Frame tde16frame{};
  for (int i = 0; i < tot_adc16_samples; i++) {
    tde16frame.set_adc_sample(0x63, i);
  }

  for (int i = 0; i < 30; i++) {
    BOOST_REQUIRE_EQUAL(tde16frame.get_adc_sample(i), 0x63);
  }
  for (int i = tot_adc16_samples - 20; i < tot_adc16_samples; i++) {
    BOOST_REQUIRE_EQUAL(tde16frame.get_adc_sample(i), 0x63);
  }
}
