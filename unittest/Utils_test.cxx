...
BOOST_AUTO_TEST_CASE(Utils_ADCOperation_LowLevelChecks)
{
  ...
  dunedaq::fddetdataformats::set_adc_2d_as_1d<uint8_t, nbytes, tiny_adc_width, adcs_per_channel, nchannels>(1, 0, 7, tiny_adc_arr);
  BOOST_REQUIRE_EQUAL(tiny_adc_arr[0], 192);
  BOOST_REQUIRE_EQUAL(tiny_adc_arr[1], 1);
  BOOST_REQUIRE_EQUAL(tiny_adc_arr[2], 0);
  BOOST_REQUIRE_EQUAL(7, (dunedaq::fddetdataformats::get_adc_2d_as_1d<uint8_t, nbytes, tiny_adc_width, adcs_per_channel, nchannels>(1, 0, tiny_adc_arr)));
...
BOOST_AUTO_TEST_CASE(Utils_ADCOperation_BasicChecks)
{
  ...
  for (int i_r = 0; i_r < nrows; ++i_r) {
    for (int i_c = 0; i_c < ncols; ++i_c) {
      BOOST_REQUIRE_EQUAL(myarr[i_r][i_c], (dunedaq::fddetdataformats::get_adc_2d<wordtype_t, nrows, ncols, bits_per_word>(i_r, i_c, myarr))); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    }
  }

  // Next, test set_adc_1d if the ADC representation aligns with the C++ type

  constexpr wordtype_t inputarr[nrows][ncols] = {{1234, 5678, 9101}, {1121, 3141, 5161}}; // NOLINT(modernize-avoid-c-arrays)
  for (int i_r = 0; i_r < nrows; ++i_r) {
    for (int i_c = 0; i_c < ncols; ++i_c) {
      dunedaq::fddetdataformats::set_adc_2d<wordtype_t, nrows, ncols, bits_per_word>(i_r, i_c, inputarr[i_r][i_c], myarr); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    }
  }

  for (int i_r = 0; i_r < nrows; ++i_r) {
    for (int i_c = 0; i_c < ncols; ++i_c) {
      BOOST_REQUIRE_EQUAL(inputarr[i_r][i_c], (dunedaq::fddetdataformats::get_adc_2d<wordtype_t, nrows, ncols, bits_per_word>(i_r, i_c, myarr))); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index) 
    }
  }
  ...
  for (int i_smp = 0; i_smp < nsamples; ++i_smp) {
    for (int i_ch = 0; i_ch < nchannels; ++i_ch) {
      dunedaq::fddetdataformats::set_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(i_smp, i_ch, inputarr2[i_smp][i_ch], myarr); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
    }
  }

  for (int i_smp = 0; i_smp < nsamples; ++i_smp) {
    for (int i_ch = 0; i_ch < nchannels; ++i_ch) {
      BOOST_REQUIRE_EQUAL(inputarr2[i_smp][i_ch], (dunedaq::fddetdataformats::get_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(i_smp, i_ch, myarr)));      
    }
  }
  ...
  BOOST_CHECK_THROW((dunedaq::fddetdataformats::get_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(0, -1, myarr)),
		    std::out_of_range);
  BOOST_CHECK_THROW((dunedaq::fddetdataformats::get_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(0, bad_channel_index, myarr)),
		    std::out_of_range);

  BOOST_CHECK_THROW((dunedaq::fddetdataformats::set_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(-1, 0, good_adc_val, myarr)),
		    std::out_of_range);
  BOOST_CHECK_THROW((dunedaq::fddetdataformats::set_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(nrows, 0, good_adc_val, myarr)),
		    std::out_of_range);
  BOOST_CHECK_THROW((dunedaq::fddetdataformats::set_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(0, -1, good_adc_val, myarr)),
		    std::out_of_range);
  BOOST_CHECK_THROW((dunedaq::fddetdataformats::set_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(0, bad_channel_index, good_adc_val, myarr)),
		    std::out_of_range);

  BOOST_CHECK_THROW((dunedaq::fddetdataformats::set_adc_2d<wordtype_t, nrows, ncols, bits_per_adc>(0, 0, bad_adc_val, myarr)),
		    std::out_of_range);
